...
class GraphWorkflowEngine:
    ...
    def _process_step(self, session_id, step_id):
        ...
        try:
            ...
            # Add context parameter if include_history is present
            if resolved_inputs and 'include_history' in resolved_inputs and resolved_inputs['include_history']:
                # If include_history is enabled, extract only the messages array rather than the full state
                # This avoids creating a cycle in the JSON and keeps the Neo4j property size manageable
                if "data" in state and "messages" in state["data"]:
                    logger.info(f"Adding messages array as context for history in step {step_id}")
                    ...
                    logger.info(f"No messages found for context in step {step_id}")
                    resolved_inputs['context'] = {"data": {"messages": []}}
